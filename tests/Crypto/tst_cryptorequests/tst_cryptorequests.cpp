...
void tst_cryptorequests::lockCode()
{
    ...
    QCOMPARE(lcr.result().errorMessage(), QStringLiteral("Crypto plugin %1 does not support locking")
                                                    .arg(plugins.value(CryptoTest::CryptoPlugin)));
...
void tst_cryptorequests::importKey_data()
{
    ...
    promptForSailfishPassphrase.setInputType(InteractionParameters::AlphaNumericInput);
    promptForSailfishPassphrase.setEchoMode(InteractionParameters::NormalEcho);
    ...
    promptToCancel.setInputType(InteractionParameters::AlphaNumericInput);
    promptToCancel.setEchoMode(InteractionParameters::NormalEcho);
